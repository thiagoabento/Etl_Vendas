...
def ajustando_valor_negativo(df):
    try:
        ...
        df.loc[:,'quantidade'] = df['quantidade'].apply(lambda x:abs(float(x)) if float(x) < 0 else float(x))
...
def convertendo_literal_em_numeral(df):
    try:
        literal = {'um': 1,'dois': 2,'três': 3,'quatro': 4,'cinco': 5,
        ...
        literal_quantidade = df['quantidade'].apply(lambda x: isinstance(x, str) and x in literal.keys()).any()
        literal_preco      = df['preco'].apply(lambda x:isinstance(x,str) and x in literal.keys()).any()
...
def incluir_coluna_total(df):
    ...
    try:
        ...
        logging.error(f'Ocorreu erro na função "inclui_coluna_total": {e}')
